...
document.body.innerHTML = "show begins";
...
function grap() {
    getImage(i++).then(clean).then(parts => {

        document.body.innerHTML = '';
        parts.map(part => {


            document.body.appendChild(part.canvas);

            (function () {
                var key = part.hash;
                var input = document.createElement('input');
                input.value = recognize(part.hash);
...
function get() {
    document.body.innerHTML = "<input value='" + JSON.stringify(map) + "'>"
}